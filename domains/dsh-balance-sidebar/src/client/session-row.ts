 ...
 */
export function sessionIdOfRow(row: HTMLElement): string | undefined {
  const fiberKey = Object.keys(row).find((key) => key.startsWith("__reactFiber$"));
  if (fiberKey === undefined) return undefined;
  let fiber = (row as unknown as Record<string, unknown>)[fiberKey] as
  ...
  for (let depth = 0; fiber !== undefined && depth < MAX_FIBER_DEPTH; depth++) {
    ...
    if (
      node !== undefined && typeof node.id === "string" && "updatedAt" in node
    ) {
      return node.id;
    }
...
function timeSpanOf(row: HTMLElement): HTMLSpanElement | null {
...
export class SessionRowCostInjector {
  private readonly entries = new Map<HTMLElement, RowEntry>();
  ...
  private render(entry: RowEntry): void {
