...
export interface BalanceSidebarSummary {
  balance: BalanceView;
...
export function buildCostMap(
  ...
  summary: BalanceSidebarSummary | undefined,
): Map<string, number> {
...
export type BalanceWorkspaceWidgetProps =
  & PropsRuntime<"sidebar.footer.action">
  & PropsLocale<typeof NS>;

/** The footer widget component. */
export function BalanceWorkspaceWidget(
  props: BalanceWorkspaceWidgetProps,
): JSX.Element {
  const { wide, useSessions, useWorkspaces, t } = props;
  const sessions = useSessions((state) => state);
  const workspaces = useWorkspaces((state) => state);
  const [summary, setSummary] = useState<BalanceSidebarSummary | undefined>(
    undefined,
  );
  ...
  return (
    <div className={css.root}>
      <div className={css.balanceRow}>
        <span className={css.balanceValue} title={balance?.error}>
          {t("widget.balance", { amount: formatBalance(balance?.total) })}
        </span>
      ...
      {rows.length === 0
        ...
        : (
          <div className={css.heatmap} role="list">
            {rows.map((row) => {
              const detail = [
                t("widget.tokens"),
              ...
              return (
                ...
                >
                  ...
                  <span className={css.cost}>{formatMoney(row.cost)}</span>
                </div>
              );
            })}
          </div>
