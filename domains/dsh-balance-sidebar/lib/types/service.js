...
import { costOfTokens, costOfUsage, FLASH_COST_CONFIG, PRO_COST_CONFIG, resolveCostConfig, } from "./cost.js";
...
function parseBaseUrl(raw) {
    ...
    if (url.protocol !== "https:" && url.protocol !== "http:") {
        throw new Error(`dsh-balance-sidebar: baseUrl must be http(s), got "${url.protocol}"`);
    }
 ...
 */
export class BalanceSidebarService extends Service {
    apiKeyEnv;
    baseUrl;
    refreshIntervalMs;
    model;
    ...
    constructor(ctx, config = {}) {
        ...
        this.refreshIntervalMs = Math.max(0, (config.refreshIntervalSeconds ?? DEFAULT_REFRESH_INTERVAL_SECONDS) *
            1_000);
