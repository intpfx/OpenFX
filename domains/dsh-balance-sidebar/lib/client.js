window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/**
		* dsh-balance-sidebar session-row cost chips — DOM-level augmentation of the
		...
		*
		* The chip markup is plain DOM (no React), keyed by `data-balance-sidebar`,
		* so re-syncs are idempotent and unload removes every chip.
		* @module dsh-balance-sidebar/client/session-row
		*/
		...
		const CHIP_ATTR = "data-balance-sidebar-chip";
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:/Users/siaovon/Documents/OpenFX/domains/dsh-balance-sidebar/src/client/widget.module.css.mjs
		const css = ".gustHq_root{box-sizing:border-box;border-top:1px solid var(--dsw-alias-border-subtle,#8080802e);width:100%;min-width:0;color:var(--dsw-alias-label-secondary,#666);user-select:none;flex-direction:column;gap:4px;padding:6px 8px;font-size:12px;line-height:16px;display:flex}.gustHq_balanceRow{align-items:center;gap:6px;display:flex}.gustHq_balanceValue{font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-primary,#222);font-weight:600}.gustHq_balanceState{text-overflow:ellipsis;white-space:nowrap;color:var(--dsw-alias-label-caption,#999);flex:1;overflow:hidden}.gustHq_error{color:var(--dsw-alias-state-error-primary,#d33);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.gustHq_empty{color:var(--dsw-alias-label-caption,#999);padding:2px 0}.gustHq_heatmap{flex-direction:column;gap:2px;width:100%;min-width:0;max-height:176px;display:flex;overflow-y:auto}.gustHq_row{box-sizing:border-box;border-radius:4px;align-items:center;gap:6px;width:100%;min-width:0;min-height:20px;padding:1px 4px;display:flex}.gustHq_row:hover{background:var(--dsw-alias-interactive-bg-hover,#8080801f)}.gustHq_swatch{border-radius:2px;flex:none;width:10px;height:10px}.gustHq_name{text-overflow:ellipsis;white-space:nowrap;color:var(--dsw-alias-label-primary,#222);flex:1;overflow:hidden}.gustHq_tokens{font-variant-numeric:tabular-nums}.gustHq_cost{font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-primary,#222);text-align:right;min-width:52px}.gustHq_totalRow{border-top:1px solid var(--dsw-alias-border-subtle,#8080802e);font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-primary,#222);justify-content:space-between;align-items:center;gap:6px;padding-top:4px;display:flex}.gustHq_rail{text-align:center;font-variant-numeric:tabular-nums;width:100%;color:var(--dsw-alias-label-secondary,#666);cursor:pointer;font-size:10px;display:inline-block}";
		const tagId = "dsh-balance-sidebar/widget.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-balance-sidebar";
			tag.dataset.pluginCss = tagId;
			tag.textContent = css;
			document.head.appendChild(tag);
		}
		var widget_module_css_default = {
			"balanceRow": "gustHq_balanceRow",
			"balanceState": "gustHq_balanceState",
			"balanceValue": "gustHq_balanceValue",
			"cost": "gustHq_cost",
			"empty": "gustHq_empty",
			"error": "gustHq_error",
			"heatmap": "gustHq_heatmap",
			"name": "gustHq_name",
			"rail": "gustHq_rail",
			"root": "gustHq_root",
			"row": "gustHq_row",
			"swatch": "gustHq_swatch",
			"tokens": "gustHq_tokens",
			"totalRow": "gustHq_totalRow"
		};
		//#endregion
		//#region src/client/BalanceWorkspaceWidget.tsx
		/**
		...
		*/
		...
		async function summaryFetch() {
			...
			if (!response.ok) throw new Error(`balance-sidebar summary failed: ${response.status}`);
		...
		*/
		...
		function BalanceWorkspaceWidget(props) {
		...
		*/
		...
		//#endregion
		exports.BalanceWorkspaceWidget = BalanceWorkspaceWidget;
