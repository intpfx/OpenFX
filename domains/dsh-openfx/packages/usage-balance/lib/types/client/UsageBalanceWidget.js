...
export function UsageBalanceWidget(props) {
    ...
    if (!wide) {
        const label = balance?.error === undefined
            ...
            : "—";
        return (_jsx("span", { className: css.rail, "data-usage-balance-value": true, title: balance?.error ??
                t("widget.balance", { amount: formatBalance(balance?.total) }), children: label }));
    }
    return (_jsxs("div", { className: css.root, "data-usage-balance-root": true, children: [_jsxs("div", { className: css.balanceRow, children: [_jsx("span", { className: css.balanceValue, "data-usage-balance-value": true, title: balance?.error, children: t("widget.balance", { amount: formatBalance(balance?.total) }) }), _jsx("span", { className: css.balanceState, children: balance?.error !== undefined
                ...
                : (_jsx("ul", { className: css.heatmap, children: rows.map((row) => {
                        const detail = [
                            t("widget.tokens"),
                        ...
                        return (_jsxs("li", { className: css.row, "data-usage-balance-workspace-row": true, title: detail, children: [_jsx("span", { className: css.swatch, style: {
                                        background: heatColor(row.totalTokens / maxTokens),
                                    } }), _jsx("span", { className: css.name, "data-usage-balance-workspace-name": true, children: row.workspaceId === undefined ? t("widget.ungrouped") : row.title }), _jsx("span", { className: css.tokens, children: formatTokens(row.totalTokens) }), _jsx("span", { className: css.cost, "data-usage-balance-workspace-cost": true, children: formatMoney(row.cost) })] }, row.workspaceId ?? "__ungrouped__"));
