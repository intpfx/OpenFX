...
function clearSearchParamsFromUrl() {
  ...
  if (!isBewlyHomePage() || isSearchResultsPage()) {
    return
  }
  ...
  const hasSearchParams = urlParams.has('keyword')
    ...
    || urlParams.has('pn')
  const hasRemovedSearchPage = urlParams.get('page') === 'Search'

  if (hasSearchParams || hasRemovedSearchPage) {
...
if (activatedPage.value !== AppPage.SearchResults && isBewlyHomePage() && !isSearchResultsPage()) {
...
const iframePageURL = computed((): string => {
  ...
  if (currentDockItemConfig) {
    return !mainStore.getDockItemByPage(activatedPage.value)?.hasBewlyPage ? mainStore.getBiliWebPageURLByPage(activatedPage.value) : ''
  }
  return ''
})
...
function handleDockItemClick(dockItem: DockItem) {
  ...
  if (dockItem.openInNewTab) {
    ...
    // Clear search keyword and URL params when switching away from search results (only on homepage)
    if (isBewlyHomePage() && !isSearchResultsPage()) {
...
<template>
  <div
    ...
    ref="mainAppRef"
    class="bewly-wrapper"
    :class="{ 'dark': isDark, 'mobile-userscript': isMobileUserscriptPage }"
  ...
  >
    ...
    <!-- Settings -->
    <KeepAlive>
      <Settings v-if="showSettings" class="bewly-settings" z-10002 @close="showSettings = false" />
    </KeepAlive>
