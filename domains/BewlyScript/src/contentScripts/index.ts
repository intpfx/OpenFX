...
import { isMobileBilibiliPage, isUserscriptRuntime, MOBILE_USERSCRIPT_SHADOW_CSS, prepareMobileUserscriptDefaults } from '~/userscript/mobile'
...
const isMobileUserscriptPage = isUserscriptRuntime() && isMobileBilibiliPage(currentUrl)
...
if (isElectronEnv) {
  ...
  // Always use dark mode if enabled, but let useDark() handle selective application
    ...
    if (isMobileUserscriptPage)
      prepareMobileUserscriptDefaults(settings.value as unknown as Record<string, unknown>)
