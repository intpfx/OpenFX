...
export interface Settings {
  ...
  dockItemsConfig: { page: AppPage, visible: boolean, openInNewTab: boolean }[]
  ...
  // Video card shadow customization
...
watch(
  ...
  (value) => {
    ...
    if (Array.isArray(record.dockItemsConfig)) {
      record.dockItemsConfig = record.dockItemsConfig.filter((dockItem: { page?: string }) => dockItem.page !== 'Search')
      record.dockItemsConfig.forEach((dockItem: { useOriginalBiliPage?: boolean }) => {
        Reflect.deleteProperty(dockItem, 'useOriginalBiliPage')
      })
    }
    if (Array.isArray(record.topBarComponentsConfig)) {
      record.topBarComponentsConfig = record.topBarComponentsConfig.filter((item: { key?: string }) => {
        return item.key !== 'favorites' && item.key !== 'history' && item.key !== 'watchLater'
      })
    }
    Reflect.deleteProperty(record, 'useOriginalBilibiliHomepage')
    Reflect.deleteProperty(record, 'useOriginalBilibiliTopBar')
    Reflect.deleteProperty(record, 'showBewlyOrBiliPageSwitcher')
    Reflect.deleteProperty(record, 'showBewlyOrBiliTopBarSwitcher')
    Reflect.deleteProperty(record, 'enableGridLayoutSwitcher')
    Reflect.deleteProperty(record, 'videoCardLayout')
    Reflect.deleteProperty(record, 'searchPageDarkenOnSearchFocus')
    Reflect.deleteProperty(record, 'searchPageBlurredOnSearchFocus')
    Reflect.deleteProperty(record, 'searchPageLogoColor')
    Reflect.deleteProperty(record, 'searchPageLogoGlow')
    Reflect.deleteProperty(record, 'searchPageShowLogo')
    Reflect.deleteProperty(record, 'searchPageSearchBarFocusCharacter')
    Reflect.deleteProperty(record, 'individuallySetSearchPageWallpaper')
    Reflect.deleteProperty(record, 'searchPageWallpaperMode')
    Reflect.deleteProperty(record, 'searchPageWallpaper')
    Reflect.deleteProperty(record, 'searchPageEnableWallpaperMasking')
    Reflect.deleteProperty(record, 'searchPageWallpaperMaskOpacity')
    Reflect.deleteProperty(record, 'searchPageWallpaperBlurIntensity')
    Reflect.deleteProperty(record, 'searchPageWallpaperCacheTime')
    Reflect.deleteProperty(record, 'useSearchPageModeOnHomePage')
    Reflect.deleteProperty(record, 'searchPageModeWallpaperFixed')
    ...
    // 迁移旧的 locallyUploadedWallpaper 到 localSettings
...
void browser.storage.local.remove(['gridBreakpoints', 'gridLayout']).catch(() => {})

export type GridLayoutType = 'adaptive'
