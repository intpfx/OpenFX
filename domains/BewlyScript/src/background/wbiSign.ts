...
function getRuntimeFetch(): RuntimeFetch {
  return (globalThis as { __BEWLYSCRIPT_FETCH__?: RuntimeFetch }).__BEWLYSCRIPT_FETCH__ ?? fetch
}
...
export async function initWbiKeys(options: { noCookie?: boolean } = {}): Promise<boolean> {
  ...
  const fetchPromise = (async () => {
    try {
      ...
      const navResponse = await getRuntimeFetch()('https://api.bilibili.com/x/web-interface/nav', {
        method: 'GET',
        headers,
