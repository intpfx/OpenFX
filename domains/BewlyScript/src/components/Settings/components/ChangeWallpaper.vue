...
function changeWallpaper(url: string) {
  ...
  settings.value.enableWallpaperMasking = !!url
...
function validateLocalWallpaper() {
  ...
  if (localWallpaper?.isLocal && localWallpaper.id) {
    if (!hasLocalWallpaper(localWallpaper.id)) {
      ...
      if (isLocalWallpaper(settings.value.wallpaper)) {
        changeWallpaper('')
      }
    }
  }
...
<template>
  <SettingsItemGroup :title="$t('settings.group_wallpaper')">
      ...
      <template #bottom>
        <div grid="~ xl:cols-5 lg:cols-4 cols-3 gap-4">
          ...
          <Tooltip placement="top" :content="localSettings.locallyUploadedWallpaper?.name || ''" aspect-video>
            ...
            <picture
              class="group"
              :class="{ 'selected-wallpaper': settings.wallpaper === (localWallpaperDisplayUrl || localSettings.locallyUploadedWallpaper?.url) }"
              aspect-video bg="$bew-fill-1" rounded="$bew-radius" overflow-hidden
              un-border="4 transparent" w-full
        ...
        <div flex items-center gap-4>
          ...
          >
            <img
              ...
              :src="settings.wallpaper" alt="" loading="lazy"
              w-full h-full object-cover
    ...
    <SettingsItem v-if="settings.enableWallpaperMasking" :title="$t('settings.wallpaper_mask_opacity')" right-width="auto">
      <div class="slider-control">
        <Slider v-model="settings.wallpaperMaskOpacity" :label="`${settings.wallpaperMaskOpacity}%`" />
      </div>
    </SettingsItem>
    <SettingsItem v-if="settings.enableWallpaperMasking" :title="$t('settings.wallpaper_blur_intensity')" right-width="auto">
      <div class="slider-control">
