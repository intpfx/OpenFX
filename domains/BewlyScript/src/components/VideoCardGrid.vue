...
const { gridCssVars } = useGridLayout()
...
watch(
  ...
  (newLen, oldLen) => {
    ...
    if (newLen > oldLen) {
      ...
      const estimatedColumns = getCurrentColumnCount(gridContainerRef.value?.clientWidth || (typeof window !== 'undefined' ? window.innerWidth : 0))
...
function getFallbackEstimatedRowHeight(): number {
  return Math.max(1, getEstimatedRowSpan() - getGridGap())
}
