...
import { isMobileUserscriptRuntimePage, normalizeBilibiliUrlForCurrentSurface, openBilibiliLoginPage, openMobileLoginDrawer, openMobileUrlInCurrentPage } from '~/userscript/mobile'
...
const { refreshUserAvatar } = topBarStore
const isMobileBottomSearch = computed(() => props.mobileBottom)
const showMobileUserPanelDrawer = ref(false)
const mobileAvatarLoadFailed = ref(false)
const mobileAccountResolving = ref(false)
...
const mobileAvatarUrl = computed(() => normalizeAvatarUrl(userInfo.value.face))
const shouldShowMobileAvatarImage = computed(() => Boolean(mobileAvatarUrl.value) && !mobileAvatarLoadFailed.value)

watch(mobileAvatarUrl, () => {
  mobileAvatarLoadFailed.value = false
})
...
function handleMobileLoginClick(event: MouseEvent) {
  ...
  if (isMobileUserscriptPage.value) {
    void resolveMobileAccountClick()
    return
  }
...
function openMobileUserPanelDrawer(event?: Event) {
  event?.preventDefault()
  event?.stopPropagation()
  topBarStore.closeAllPopups()
  showMobileUserPanelDrawer.value = true
}

function closeMobileUserPanelDrawer() {
  showMobileUserPanelDrawer.value = false
}
...
function handleMobileAvatarClick(event: MouseEvent) {
  ...
  openMobileUserPanelDrawer(event)
  refreshUserAvatar()
}

async function resolveMobileAccountClick() {
  if (mobileAccountResolving.value)
    return

  mobileAccountResolving.value = true
  try {
    await topBarStore.getUserInfo()
    if (topBarStore.isLogin) {
      refreshUserAvatar()
      topBarStore.startUpdateTimer()
      openMobileUserPanelDrawer()
      return
    }
    ...
    mobileAccountResolving.value = false
  }
}
</script>
...
<template>
  ...
  >
        ...
        >
          ...
          >
            <button
              ...
              :disabled="mobileAccountResolving"
            ...
            <button
              ...
              :disabled="mobileAccountResolving"
              :style="{
                backgroundImage: shouldShowMobileAvatarImage ? `url(${mobileAvatarUrl})` : undefined,
              }"
              @pointerdown="handleMobileAccountPointerDown"
              @mousedown="handleMobileAccountPointerDown"
              @touchstart.stop
            ...
            >
              <img
                v-if="shouldShowMobileAvatarImage"
                :src="mobileAvatarUrl"
                :alt="userInfo.uname || '用户头像'"
                class="mobile-search-account-button__avatar-img"
                referrerpolicy="no-referrer-when-downgrade"
                @error="mobileAvatarLoadFailed = true"
              >
              <span
                v-else
                class="mobile-search-account-button__fallback"
    ...
    <template v-if="isMobileUserscriptPage && isLogin">
      <Transition name="fade">
        <div
          v-if="showMobileUserPanelDrawer"
          class="mobile-search-user-panel-mask"
          @click.stop="closeMobileUserPanelDrawer"
        />
      </Transition>

      <Transition name="mobile-user-panel">
        <UserPanelPop
          v-if="showMobileUserPanelDrawer"
          :user-info="userInfo"
          class="bew-popover"
...
.mobile-search-account-button__avatar-img {
  display: block;
  width: 100%;
  height: 100%;
...
.mobile-search-account-button__fallback {
  color: var(--bew-text-3);
}

.mobile-search-user-panel-mask {
  position: fixed;
  inset: 0;
  z-index: 10020;
