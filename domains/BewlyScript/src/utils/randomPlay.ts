...
export function getRandomNextEpisode(episodes: HTMLElement[], currentIndex: number): number {
  console.log('[BewlyScript Random Play] getRandomNextEpisode called, currentIndex:', currentIndex, 'visitedEpisodes:', Array.from(visitedEpisodes))
  ...
  if (visitedEpisodes.size >= episodes.length) {
    console.log('[BewlyScript Random Play] All episodes visited, resetting')
  ...
  console.log('[BewlyScript Random Play] Unvisited indices:', unvisitedIndices)

  if (unvisitedIndices.length === 0) {
    console.log('[BewlyScript Random Play] No unvisited indices, selecting random excluding current')
    ...
    if (availableIndices.length === 0) {
      console.log('[BewlyScript Random Play] No available indices')
      return currentIndex
    }
    const selected = availableIndices[Math.floor(Math.random() * availableIndices.length)]
    console.log('[BewlyScript Random Play] Selected from available:', selected)
    return selected
  }

  const selected = unvisitedIndices[Math.floor(Math.random() * unvisitedIndices.length)]
  console.log('[BewlyScript Random Play] Selected from unvisited:', selected)
  return selected
}
...
export function jumpToEpisode(episodes: HTMLElement[], targetIndex: number): void {
  ...
  console.log('[BewlyScript Random Play] Target episode element:', targetEpisode)
  ...
  console.log('[BewlyScript Random Play] Found link:', link, 'href:', link?.href)
  ...
  if (link && link.href) {
    ...
    console.log('[BewlyScript Random Play] Found .simple-base-item:', simpleBaseItem)
  ...
  if (!clickableElement) {
    console.log('[BewlyScript Random Play] No clickable element found')
    return
  }

  console.log('[BewlyScript Random Play] Clickable element:', clickableElement)

  // 使用更智能的点击策略
  const performClick = () => {
    try {
      console.log('[BewlyScript Random Play] Attempting to click element:', targetIndex, clickableElement)
      ...
      if (clickableElement instanceof HTMLAnchorElement && clickableElement.href) {
        console.log('[BewlyScript Random Play] Clicking anchor element, href:', clickableElement.href)
        clickableElement.click()
        return
      }

      // 对于没有链接的元素（如 video-pod__item），模拟真实点击
      console.log('[BewlyScript Random Play] Simulating click on element')
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            console.log('[BewlyScript Random Play] Click sequence completed')
          }, 10)
        }, 10)
      }, 10)
    }
    catch (error) {
      console.error('[BewlyScript Random Play] Click failed:', error)
    }
  }
...
export function enableRandomPlay(): void {
  ...
  const setupVideoListener = () => {
    ...
    const randomPlayListener = () => {
      console.log('[BewlyScript Random Play] Video ended, random play listener triggered')

      // 防止重复触发（ended 和 pause 事件可能都会触发）
      if (isProcessing) {
        console.log('[BewlyScript Random Play] Already processing, skipping')
        return
      }

      // 关键：检查随机播放是否仍然启用
      if (!isRandomPlayEnabled) {
        console.log('[BewlyScript Random Play] Random play disabled, skipping')
        return
      }
      ...
      setTimeout(() => {
        ...
        // 再次检查状态，防止在延迟期间被禁用
        if (!isRandomPlayEnabled) {
          console.log('[BewlyScript Random Play] Random play disabled during delay, skipping')
          return
        }

        const episodes = getVideoEpisodes()
        console.log('[BewlyScript Random Play] Found episodes:', episodes.length)

        if (episodes.length <= 1) {
          console.log('[BewlyScript Random Play] Not enough episodes')
          return
        }

        const currentIndex = getCurrentEpisodeIndex(episodes)
        console.log('[BewlyScript Random Play] Current episode index:', currentIndex)

        const nextIndex = getRandomNextEpisode(episodes, currentIndex)
        console.log('[BewlyScript Random Play] Next episode index:', nextIndex)

        if (nextIndex !== currentIndex) {
          jumpToEpisode(episodes, nextIndex)
        }
        else {
          console.log('[BewlyScript Random Play] Next index same as current, not jumping')
        }
