...
- 转发请求方法、请求体和大部分请求头。
- 重写请求 `origin` / `referer`。
- 为响应添加 CORS 头，移除 `x-frame-options`。
...
```text
server/
  handler.ts  # 代理配置、目标 URL 构造、请求转发与响应头改写
```

Nitro API 路由位于 `entry/web/server/routes/api/proxy/[...path].ts`，通过
`proxyRequest()` 装配本 domain。
...
当前本 domain 尚未有独立测试文件。修改代理逻辑后应至少运行：
