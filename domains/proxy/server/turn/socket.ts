...
export function createNodeSocket(family: 'udp4' | 'udp6'): UdpSocket {
  ...
  return {
    ...
    send(msg: Uint8Array, offset: number, length: number, port: number, address: string): Promise<void> {
    ...
    on(event: 'message' | 'error' | 'listening' | 'close', handler: (...args: unknown[]) => void) { raw.on(event, handler as (...args: unknown[]) => void); },
    address() { return raw.address(); },
    bind(opts: { address: string; port: number; exclusive?: boolean }) { raw.bind(opts); },
    get raw() { return raw; },
  };
}
...
export function createDenoSocket(family: 'udp4' | 'udp6'): UdpSocket {
  ...
  (async () => {
    try {
      ...
      addrInfo = { address: addr.hostname, family: addr.hostname.includes(':') ? 'IPv6' : 'IPv4', port: addr.port };
    } catch { /* ignore */ }
    ...
    for await (const [data, rinfo] of conn) {
      ...
      if (msgListeners) {
        for (const h of msgListeners) {
          try { h(data, { address: remote.hostname, port: remote.port }); } catch { /* skip */ }
        }
      }
    }
  })().catch((err) => {
  ...
  return {
    get remoteAddress() { return undefined; },
    get remotePort() { return undefined; },
    async send(msg: Uint8Array, _offset: number, _length: number, port: number, address: string) {
    ...
    close() { closed = true; try { conn.close(); } catch { /* ignore */ } },
    on(event: 'message' | 'error' | 'listening' | 'close', handler: (...args: unknown[]) => void) { _on(event, handler); },
    address() { return addrInfo; },
 ...
 */
export function createUdpSocket(ctx: { debugLevel: number }, family: 'udp4' | 'udp6'): UdpSocket {
