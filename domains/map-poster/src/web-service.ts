...
export type { MapPosterResolvedPlace } from "./types.ts";
...
export type MapPosterDependencies = {
  geocode?: (
    city: string,
    country: string,
...
function readDirectCenter(input: MapPosterRenderRequest) {
  ...
  if (longitude === undefined) {
    throw new MapPosterInputError("invalid_longitude");
  }
...
export async function createMapPoster(
  ...
  const center = options.directCenter ??
    await geocodePlace(options.city, options.country, deps.geocode);
  const shouldResolvePlace = Boolean(
    options.directCenter && !options.displayCity && !options.displayCountry,
  );
  const placePromise = shouldResolvePlace && deps.reverseGeocode
    ? deps.reverseGeocode(center).catch(() => undefined)
