...
图纸中读取实体颜色、文字和块属性，按燃气工程规则汇总材料类型、材料名称、管径、数量或长度，并导出 CSV。
...
- `DN` 默认架空时，纯数字文本归类为 `镀锌钢管`，含 `W-` 归类为 `无缝焊接钢管`，含 `Y-`
  归类为 `有缝焊接钢管`。
- `RGB:127,255,223` 按原规则识别为 `自供材`，其余有效汇总行统一标注为 `甲供材`。
- `CY` 归类为 `穿越`，`SZ` 归类为 `水钻`。
- `立柱` 单独统计，`合计值` 记录米数，`备注` 仅在有根数时记录为 `N根`。
- 非 `立柱` 且包含 `*` 的文本按数量类管件聚合。
- 材料名称末尾的规格后缀会归入管径列，例如 `电熔变径/32`。
