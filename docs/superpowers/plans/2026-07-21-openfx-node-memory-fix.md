...
- New preferences default to `reduceMotion: true`; an explicitly persisted `false` value
  must still parse as `false` and must not be rewritten merely because production forces
...
- Produces:
  `deriveCoreMotionPolicy(requestedReduceMotion: boolean, animatedCoreAvailable: boolean): CoreMotionPolicy`.
- Produces: `CoreMotionPolicy` with exact fields `mode`, `reduceMotion`,
  `controlAvailable`, and `status`.
- Produces: control-panel presentation fields `motionStatus` and
...
Use the policy in `main.ts` for `CoreCanvasMetrics.reduceMotion`; do not overwrite
persisted preferences. Derive presentation state with `animatedCoreAvailable=false`.
...
- Produces: `ProcessMemorySnapshot` with nullable `physicalFootprintBytes`,
  `ioAcceleratorVirtualBytes`, and `ioAcceleratorRegionCount`.
- Produces: `parseVmmapSummary(text: string): ProcessMemorySnapshot` with binary K/M/G
  unit conversion.
...
assertEquals(
  parseVmmapSummary("Physical footprint: 512M").ioAcceleratorRegionCount,
  null,
);
```
...
1. Launch the same `.app` through Launch Services with the unique token and marker
   paths.
2. Verify the marker PID, executable real path, `ps` command token, and `lsof` text
   mapping before sampling.
3. Wait for health, then warm up for exactly 30,000 ms.
4. Run `/usr/bin/vmmap -summary <verified pid>` once for the baseline and every 30,000
   ms for 20 more samples.
...
- Fresh pinned Perry clone and
  `deno task perry:runtime --source /tmp/perry-openfx-memory-fix`
