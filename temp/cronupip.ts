...
type OSName = "darwin" | "linux" | "windows" | "freebsd" | "netbsd" | "aix" | "solaris" | "unknown";
...
function promptText(message: string, defaultValue?: string): string {
	const shown = defaultValue !== undefined && defaultValue !== "" ? `${message} (default: ${defaultValue})` : message;
...
function isLinkLocalIpv6(addr: string): boolean {
	const a = normalizeIpv6(addr);
	return a.startsWith("fe80:");
}

function isUniqueLocalIpv6(addr: string): boolean {
	const a = normalizeIpv6(addr);
...
function isValidEndpointKey(key: string): boolean {
	// key 会映射成服务端路由段：/key/...
	// 禁止保留字 update；且只允许安全字符
	if (!key) return false;
	if (key.toLowerCase() === "update") return false;
	return /^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/.test(key);
}

function parsePortOrNull(s: string): number | null {
	const v = Number(s);
...
function normalizeServerBaseUrl(input: string): string {
	const s = input.trim();
	if (!s) return "";
	// 允许用户输入 https://host 或 https://host/update
	try {
		const u = new URL(s);
		// 去除尾部斜杠
...
function isValidServerUrl(input: string): boolean {
	try {
...
function computeUpdateUrl(baseOrUpdateUrl: string): string {
	const normalized = normalizeServerBaseUrl(baseOrUpdateUrl);
	try {
		const u = new URL(normalized);
		if (u.pathname.toLowerCase().endsWith("/update") || u.pathname.toLowerCase() === "update") {
			return u.toString();
		}
		u.pathname = `${u.pathname || ""}/update`.replace(/\/\/+/, "/");
		return u.toString();
	} catch {
		// 不合法 URL：原样拼接（尽量不报错）
...
async function getHostname(timeoutMs: number): Promise<string | null> {
	const os = getOsName();
	if (os === "windows") {
		return Deno.env.get("COMPUTERNAME") ?? (await runCmdText(["hostname"], timeoutMs))?.trim() ?? null;
	}
...
function pickPreferredIpv6(list: string[]): string | null {
	const sorted = [...list].sort((a, b) => {
		const score = (x: string) => {
			if (isGlobalUnicastIpv6(x)) return 0;
			if (isUniqueLocalIpv6(x)) return 1;
			if (isLinkLocalIpv6(x)) return 3;
			return 2;
		};
...
async function postIpv6Report(ipv6: string, options: Options): Promise<PostResult> {
	...
	if (!res.ok) {
		throw new Error(
			`POST failed: ${res.status} ${res.statusText}${responseText ? ` - ${responseText}` : ""}`,
		);
	}
...
function printHelp(): void {
	...
	const base = exec.split(/[/\\]/).filter(Boolean).pop() ?? "cronupip";
	const isDeno = /(^|[/\\])deno(\.exe)?$/i.test(exec);
	const usage = isDeno
		? "deno run -A cronupip.ts"
		: base;
...
Examples:
	deno run -A --unstable-cron cronupip.ts
	deno run -A --unstable-cron cronupip.ts --server https://example.com --key home --port 80
	deno compile -A --unstable-cron --output cronupip cronupip.ts && ./cronupip
...
function buildOptions(): Options | null {
	...
	const envTimeoutMs = Number.isFinite(timeoutEnv) && timeoutEnv > 0 ? Math.floor(timeoutEnv) : undefined;
...
async function assertPublicIpv6Support(timeoutMs: number): Promise<{ ipv6List: string[]; publicIpv6List: string[] } | null> {
	...
	while (true) {
		...
		while (!endpointKey || !isValidEndpointKey(endpointKey)) {
			...
			if (endpointKey.includes("://")) {
				console.log("That looks like a URL. The key should be a short name like 'home' or 'nas'.");
			}
		...
		while (!(endpointPort >= 1 && endpointPort <= 65535)) {
			const entered = promptRequired("[3/3] Target port (1-65535, e.g. 80/443/3000)", endpointPort ? String(endpointPort) : "80");
...
async function main(): Promise<void> {
	...
	if (typeof (Deno as unknown as { cron?: unknown }).cron !== "function") {
		...
		console.error("If you run from source: use deno run -A --unstable-cron cronupip.ts");
		console.error("If you compile: use deno compile -A --unstable-cron --output cronupip cronupip.ts");
		Deno.exit(2);
	}
