...
} from "../../domains/map-poster/src/viewport.ts";
import {
...
Deno.test("map poster viewport keeps center math deterministic", () => {
  const center = { lat: 31.2304, lon: 121.4737 };
  const unchanged = moveMapCenterByPixels(center, 12, 0, 0);
  expect(unchanged.lat).toBeCloseTo(center.lat, 10);
  expect(unchanged.lon).toBeCloseTo(center.lon, 10);

  const wrapped = moveMapCenterByPixels({ lat: 0, lon: 179.9 }, 3, 32, 0);
  expect(wrapped.lat).toBeCloseTo(0, 8);
  expect(wrapped.lon).toBeGreaterThanOrEqual(-180);
  expect(wrapped.lon).toBeLessThan(180);
});

Deno.test("map poster viewport wraps horizontal tiles and clips polar rows", () => {
  const tiles = getVisibleMapTiles(
    { lat: 85.05112878, lon: 179.9 },
    2,
    { width: 512, height: 512 },
  );
  expect(tiles.length).toBeGreaterThan(0);
  expect(tiles.some((tile) => tile.src.includes("/2/0/"))).toBe(true);
  expect(
    tiles.every((tile) => {
      const tileY = Number(tile.src.split("/").at(-1)?.replace(".png", ""));
      return tileY >= 0 && tileY < 4;
    }),
  ).toBe(true);
});
...
Deno.test("map poster Nominatim endpoints use local defaults and both fail closed in production", () => {
  ...
  expect(resolveReverseGeocodeEndpoint({ NODE_ENV: "production" }))
    .toBeUndefined();
  expect(
...
Deno.test("map poster Nominatim broker spaces aggregate calls and deduplicates concurrent reverse work", async () => {
  ...
  await expect(Promise.all([first, duplicate, search, second])).resolves
    .toEqual([
