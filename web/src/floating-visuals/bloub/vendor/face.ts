...
export function eyePoses(
  ...
  let down: Vec3 = [0, 1, 0]; // lacet : forward bascule vers right
  [f, right] = spin(f, right, deg(gaze.yaw)); // tangage : forward bascule vers le haut (donc a l'oppose de down)
  [down, f] = spin(down, f, deg(gaze.pitch)); // roulis : la tete penche dans son propre plan
