import { type ArcRender, arcRender, type DotRender } from "./decor.ts";
...
import { type Pose, STATE_BY_ID, type StateDef, type StateId } from "./states.ts";
    ...
    if (pose.eyeAlpha > 0.01) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        eyes.push({
          ...
          matrix: `matrix(${r2(ax)},${r2(ay * k)},${r2(cx2)},${r2(cy2 * k)},${
            r2(e.x * fit + (offX + decalage.x) * R)
          },${r2(e.y * fit + (offY + decalage.y) * R)})`,
