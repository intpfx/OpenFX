...
import { type BotExpression, EXPRESSIONS } from "./expressions.ts";
import { eyePoses } from "./face.ts";
import { type Point, radiusAtAngle, toPoints } from "./shape.ts";
import { SHAPES } from "./skins.ts";
import { type Pose, type StateDef, type StateId, STATES } from "./states.ts";
...
function pire(pts: Point[], emps: Empreinte[], tx: number, ty: number) {
  ...
  for (const e of emps) {
    ...
    const rayon = e.r * Math.hypot(m0 * a.ux + m1 * a.uy, m2 * a.ux + m3 * a.uy) +
      FLOTTEMENT;
    if (a.d - rayon < marge) {
      marge = a.d - rayon;
      ux = a.ux;
