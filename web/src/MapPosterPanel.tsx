...
import type { MapPosterRenderResult } from "@domains/map-poster/src/web-service.ts";
...
const DEFAULT_MAP_CENTER: Coord = { lat: 35.6768601, lon: 139.7638947 };
...
function formatCoord(coord: Coord) {
...
export function MapPosterPanelContent() {
  ...
  const [mapCenter, setMapCenter] = useState<Coord>(DEFAULT_MAP_CENTER);
  ...
  function markMapCenterChanged(center: Coord) {
  ...
  function commitMapCenter(center: Coord) {
    setMapCenter(center);
  ...
  function handleMapKeyDown(event: KeyboardEvent<HTMLDivElement>) {
    ...
    event.preventDefault();
    commitMapCenter(
      moveMapCenterByPixels(mapCenter, mapZoom, delta[0], delta[1]),
    );
  }
