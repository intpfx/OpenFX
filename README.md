...
Web 页面另有一个可拖动的 `FloatingVisualDock` 悬浮插件台，与文件库业务组件并列组合。
插件台只负责位置、视口约束、来源和 renderer 装配；位置偏好单独保存在
`localStorage`，插件不能读取文件索引、OPFS、session 或私有网络密钥。当前两个独立插件
分别移植 Bloub 的纯 TypeScript + SVG 引擎（自动循环 14 个目录态和 `swirl` 转场态）与
Nebula-Orb 的 Canvas 点云引擎（自动循环 12 个官方状态），两者不共享形态或动画时钟。每个
renderer 只在页面可见且自身位于视口内时运行，卸载会清理自己的帧循环；系统要求减少
动态效果时固定为首帧。上游项目、版权、MIT 许可及 Bloub 的设计权利边界保存在根 `NOTICE`。
