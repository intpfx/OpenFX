...
`client.ts` 中的 `import "./assets/styles.css"` 被 Vite 打包为 JS 注入的 `<style>` 标签，在页面加载后追加到 `<head>`。由于同名选择器按 DOM 顺序后者胜出，`styles.css` 的暗色主题覆盖了首页的浅色设计。
...
**根因**：`background: var(--bg) !important` 是 CSS shorthand，包含 `background-size: initial !important`。后续的 `background-size: 80px 80px`（无 `!important`）被吃掉。

**正确写法**：只用 `background-color: var(--bg) !important`，不要用 shorthand `background`。

#### 全局字体：Inter 泄漏

`computed htmlFontFamily: "Inter, ui-sans-serif, ..."` — 来自 styles.css 的 `:root { font-family: Inter, ... }`。

**根因**：`styles.css` 用 `:root` 选择器（= `html` 选择器，同优先级），通过 Vite JS 后注入（DOM 顺序后者胜）。`homepage.css` 的 `html, body { font-family: system-ui }` 无 `!important`，被覆盖。
...
Clash Verge Rev TUN 模式（fake-IP `198.18.0.1/30`）阻断了字体下载。浏览器等待超时，`loadComplete` 延迟至 10,124ms。真实浏览器中超时通常 30 秒以上。
...
动画由 `setTimeout(tick, 16)` 驱动，不是 `requestAnimationFrame`（bb-browser 不支持 rAF）。
...
`AppShell` 只渲染 `<main class="page">`，样式由 `client.ts` 中 import 的 `styles.css` 控制。
