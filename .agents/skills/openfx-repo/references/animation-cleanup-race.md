...
`clearInterval()` 只是阻止**新**回调加入事件队列，但不影响**已经排入队列**的回调。如果
`clearInterval` 之前刚有回调被排入（JavaScript 是单线程，但 `setInterval`
的回调在下一个事件循环 tick 执行），该回调会在 `finishAnim` 清空 textContent
之后执行，把噪声字符写回去。
