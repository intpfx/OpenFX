...
从 `intpfx/dss` 项目提取的 DenoKV SSE 流式推送模式。**该模式已在 `domains/_shared/kv.ts` 中实现为 `streamKvEntries()` 函数**，不再需要手工拼凑。
...
`streamKvEntries(kv, prefix)` 返回 `ReadableStream<Uint8Array>`，SSE 格式每行 `data: {"key":...,"value":...}\n\n`。
    ...
    for await (const entry of entries) {
      ...
      const task = (async () => {
        ...
        for await (const change of watcher) {
          const data = `data: ${JSON.stringify({ key: change[0].key, value: change[0].value })}\n\n`;
          controller.enqueue(encoder.encode(data));
        }
      })();
...
`ScopedKv` 已有 get/set/delete/list，没有流式能力。
需要实时推送时可在 `_shared/kv.ts` 中新增：
```ts
