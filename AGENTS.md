...
- App 内容、catalog renderer、`App.tsx` 组件分支和测试必须保持一致；不要恢复平行的 App
  ID 列表。
- 浮动视觉插件台在 `App.tsx` 组合层与文件库并列，不向 `FileLibraryHomepage` 注入专用
  视觉插槽。宿主只负责插件描述、拖动、位置约束和来源展示；插件 renderer 不得读取文件
  索引、OPFS、session 或私有网络密钥，也不能把两个插件合成同一形态状态机。每个 renderer
  自行管理可见性、减少动态效果和卸载清理，不能引入全局永久运行的帧循环；移植上游视觉时
  保留其来源、版权、许可证和明确的设计权利边界。
